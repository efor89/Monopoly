...
class BauenUI{
	...
	public function BauenUI($player){
		...
		$form = $api->createCustomForm(function (Player $player, array $data = null) {
			...
			if($this->isHypothek($data[1]) === "yes"){
				$player->sendMessage("§bMono§6poly: §cDu kannst hier kein Haus Bauen da auf der Strasse eine Hypothek ist.");
				return true;
			}
			if($this->plugin->isFullStreet($player, $data[1]) === "no"){
				$player->sendMessage("§bMono§6poly: §cDu kannst hier kein Haus Bauen da du die Farbgruppe nicht komplett besitzt.");
				return true;
			}
			if($gamecfg->get($data[1]."haus") > 4){
				$player->sendMessage("§bMono§6poly: §cDu kannst hier kein Haus Bauen da auf der Strasse schon ein Hotel steht.");
				return true;
			}
			$playerMoney = EconomyAPI::getInstance()->myMoney($player);
			$kosten = $config->getNested($data[1].".house");
			if($playerMoney < $kosten){
				$player->sendMessage("§bMono§6poly: §cDu hast nicht genug Geld um ein Haus zu bauen.");
				return true;
			}
			EconomyAPI::getInstance()->reduceMoney($player, $kosten);
			...
			if($gamecfg->get($data[1]."haus") < 5){
			    $player->getLevel()->setBlock(new Vector3($x, $y, $z), Block::get(236, 5));
			}else{
